...
class Login {
	...
	static function isUserDeleted() {
		...
		return (Session::is('uid') && UserGateway::isDeleted(Session::uid()));
	}
	...
	static function isLoggedIn() {
		return (static::isLoggedInWithGoogle() || static::isLoggedInAsAdmin() || static::isLoggedInUser());
	}
	...
	static function isLoggedInAsStudent() {
		return (Session::is('uid') && Session::userType() == 'student' && UserGateway::isActive(Session::uid()));
	...
	static function isLoggedInAsTeacher() {
		return (Session::is('uid') && Session::userType() == 'teacher' && UserGateway::isActive(Session::uid()));
	...
	static function isLoggedInUser() {
		return (Session::is('uid') && UserGateway::isActive(Session::uid()));
	}
	...
	static function processLogin($email, $password,$ajax=true) {
		...
		if ($user) {
			if ($user['status']!='active') {
				$error = 'Account is not active. if you did\'nt received verification email <a href="'.Core::cadenzaUrl('pages/signup/regenerate_token.php').'">Resend</a>';
				if ($ajax) {
					print json_encode(array('result'=>$result,'destination'=>$destination,'loginsuccess'=>false,'error'=>$error));
					exit();
				}else{
					return $response;
				}
				...
				if (password_verify($password, $password_hash)) {
					...
					elseif ($user['user_type'] == 'teacher') {
						...
						$redirect_uri = Core::cadenzaUrl('pages/signup/index.php');
					}
					$result = 'redirect';
					$destination = filter_var($redirect_uri, FILTER_SANITIZE_URL);
					$loginsuccess = true;
	...
	static function swtichAccount($uid, $currentuid,$ajax=true) {
		$user = UserGateway::find($uid);
		$result = 'success'; // ajax result status
		$destination = null; // redirect destination
		$loginsuccess = false; // whether or not the login was successful
		if ($user) {
			if ($currentuid==$user['link_account']) {
				Session::set('uid', $user['uid']);
				Session::set('user_type', $user['user_type']);
				if ($user['user_type'] == 'student') {
					$redirect_uri = Core::cadenzaUrl('pages/student/index.php');
		...
		$response = array('result'=>$result, 'destination'=>$destination, 'loginsuccess'=>$loginsuccess,'user'=>$user);
		// Generate an AJAX response
		if ($ajax) {
			print json_encode($response);
		}else{
