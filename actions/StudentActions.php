...
class StudentActions extends UserActions {
	...
	static function addAnnotatorToPractice() {
		$file_id = isset($_REQUEST['annotator_file_id']) ? $_REQUEST['annotator_file_id'] : null;
		...
		if ($practice_row) {
			...
			if ($user->isAllowedAddAnnotatorToPractice($practice)) {
				...
				if (isset($response['files'][0]->id)) {
					...
					PracticeGateway::updateAnnotator($practice_id, $file_id, $annotator_title);
					
					// $teacher_row = UserGateway::find($practice->teacher_id);
					// if ($teacher_row) {
					// 	$teacher = new User($teacher_row);
					// 	// share with teacher
					// 	DriveAPIPermission::createPermission($service, $file_id, $teacher->email, "user", "writer");
					// }
					// TODO: place this refresh in an "if practicelog" (if it's possible to determine here)
					$response['refresh'] = array(
						'misc/practicelog_indicators'=>Components::renderMiscPracticelogIndicators($user->uid, $practice_id)
					...
					return array('result'=>'redirect', 'destination'=>filter_var($redirect_uri, FILTER_SANITIZE_URL), 'message'=>'File not uploaded', 'disable_unsafe_navigation'=>true);
				}
			}
		}
		else {
